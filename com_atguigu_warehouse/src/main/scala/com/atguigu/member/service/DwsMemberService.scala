...
object DwsMemberService {
  ...
  def importMemberUseApi(sparkSession: SparkSession, dt: String) = {
    ...
    //        result.show()
    resultData.show()
    //    resultData.write.mode(SaveMode.Append)insertInto("dws.dws_member")
  ...
  def importMember(sparkSession: SparkSession, time: String) = {
    ...
    sparkSession.sql("select uid,first(ad_id),first(fullname),first(iconurl),first(lastlogin)," +
      ...
      s"from " +
      s"dwd.dwd_member a left join dwd.dwd_member_regtype b on a.uid=b.uid " +
      ...
      "group by uid,dn,dt").coalesce(1).write.mode(SaveMode.Append).insertInto("dws.dws_member")
